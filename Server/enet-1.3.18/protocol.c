...
size_t
enet_protocol_command_size (const enet_uint8 commandNumber)
{
    return commandSizes [commandNumber & ENET_PROTOCOL_COMMAND_MASK];
}

static void
enet_protocol_change_state (ENetHost * host, ENetPeer * peer, const ENetPeerState state)
{
...
static void
enet_protocol_dispatch_state (ENetHost * host, ENetPeer * peer, const ENetPeerState state)
{
...
enet_protocol_find_sent_reliable_command (ENetList * list, const enet_uint16 reliableSequenceNumber, const enet_uint8 channelID)
{
...
enet_protocol_remove_sent_reliable_command (ENetPeer * peer, const enet_uint16 reliableSequenceNumber, const enet_uint8 channelID)
{
    ...
    if (channelID < peer -> channelCount)
    {
       ...
       const enet_uint16 reliableWindow = reliableSequenceNumber / ENET_PEER_RELIABLE_WINDOW_SIZE;
...
enet_protocol_handle_acknowledge (ENetHost * host, ENetEvent * event, ENetPeer * peer, const ENetProtocol * command)
{
    ...
    if (peer -> lastReceiveTime > 0)
    {
       ...
       {
          const enet_uint32 diff = roundTripTime - peer -> roundTripTime;
          peer -> roundTripTimeVariance += diff / 4;
          peer -> roundTripTime += diff / 8;
       }
       else
       {
          const enet_uint32 diff = peer -> roundTripTime - roundTripTime;
          peer -> roundTripTimeVariance += diff / 4;
          peer -> roundTripTime -= diff / 8;
       }
    }
...
enet_protocol_check_outgoing_commands (ENetHost * host, ENetPeer * peer, ENetList * sentUnreliableCommands)
{
    ...
    for (;;)
    {
       ...
       if (outgoingCommand -> command.header.command & ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE)
       {
          ...
          if (outgoingCommand -> packet != NULL)
          {
             const enet_uint32 windowSize = peer -> packetThrottle * peer -> windowSize / ENET_PEER_PACKET_THROTTLE_SCALE;
...
static int
enet_protocol_send_outgoing_commands (ENetHost * host, ENetEvent * event, const int checkForTimeouts)
{
